...
namespace SistemaInmueble.BLL.Implementacion
{
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<bool> CambiarClave(int IdUsuario, string ClaveActual, string ClaveNueva)
        {
            try
            {
                Usuario usurio_encontrado = await _repositorio.Obtener(u => u.IdUsuario == IdUsuario);

                if(usurio_encontrado == null)
                    throw new TaskCanceledException("El usuario no existe");

                if(usurio_encontrado.Clave != _utilidadesService.ConvertirSha256(ClaveActual))
                    throw new TaskCanceledException("La contraseña ingresada como actual no es correcta");

                usurio_encontrado.Clave = _utilidadesService.ConvertirSha256(ClaveNueva);

                bool respuesta = await _repositorio.Editar(usurio_encontrado);
        ...
        public async Task<bool> RestablecerClave(string Correo, string UrlPlantillaCorreo)
        {
            try
            {
                ...
                if(!correo_enviado)
                    throw new TaskCanceledException("Tenemos problemas. Por favor inténtalo de nuevo o más tarde");
